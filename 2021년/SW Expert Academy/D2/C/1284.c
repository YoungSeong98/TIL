...
main()
{
	int test_case, i, a_fee, b_min, b_std, b_add, water_use, a_use, b_use;

	printf("Enter a test_case : ");
	scanf("%d", &test_case);

	for (i = 0; i < test_case; i++) {
		a_use = 0;
		b_use = 0;
		printf("A`s fee, B`s minimum fee, B`s standard water use, B`s additional fee, and water use amount : ");
		scanf("%d %d %d %d %d", &a_fee, &b_min, &b_std, &b_add, &water_use);
